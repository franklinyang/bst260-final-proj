...
As we began exploring the CMS data, we looked at several histograms and scatterplots of measurements that seemed interesting. We began by looking at inpatient and total spending. 

Below is a histogram of Inpatient Spending per Claim:
```{r}
summary(master$ip_spend); sd(na.omit(master$ip_spend)) #Median 11,482; sd 2,651.519
hist(master$ip_spend)
```

We noticed that inpatient spending was skewed right, and tried a log base 10 transformation, which showed a more normal distribution:
```{r}
hist(log10(master$ip_spend))
```


We then looked at how inpatient spending might be related to hospital level complications. As there appeared to be a trend, this was something that we investigated as part of our final analysis. We also noticed that there was a potential difference in the ownership of each hospital, which is why it is included in our visualizations. 
```{r}
summary(master$hospital_level_complications_score); sd(na.omit(master$hospital_level_complications_score))
master %>% ggplot(aes(ip_spend, hospital_level_complications_score))+
  geom_point(aes(color = hospital_ownership))
```

We did think the overall trend was linear, but with a slight quadratic appearance of inpatient spending. When we began linear regression we tested linear inpatient spending against both linear and quadratic inpatient spending and found that both were significant.
```{r}
mod_simple_linear_complications <- lm(hospital_level_complications_score ~
                                        ip_spend,
                                      data = master)
mod_simple_quad_complications <- lm(hospital_level_complications_score ~
                                      ip_spend +
                                      I(ip_spend^2),
                                    data = master)
summary(mod_simple_linear_complications)
summary(mod_simple_quad_complications)
```


We were particularly interested in what the situation might be in Massachusetts and found a remarkably linear trend in spending vs. complications which prompted further analysis. 
```{r}
master %>% filter(state == 'MA') %>% ggplot(aes(ip_spend, hospital_level_complications_score))+
  geom_point(aes(color = hospital_ownership))
```

We were also interested in looking at excess hospital readmissions, because this has been an important news topic in reducing hospital spending. There was no clear relationship between total spending and excess readmissions, even after faceting by several other factors. Below is one example:
```{r}
master %>% ggplot(aes(total_spend, excess_readmissions))+
  geom_point(aes(color = hospital_ownership))+
  facet_wrap(.~hc_policy_focused_state)
```

We did begin to think that we were missing valuable information and potential confounders and added the census data. Here we found that income was an important potential confounder. 
```{r}
master %>% ggplot()+
  geom_histogram(aes(median_household_income),color="black")
mod_with_confounding <- lm(hospital_level_complications_score ~
                                      ip_spend +
                                      I(ip_spend^2)+
                                      median_household_income,
                                    data = master)
summary(mod_with_confounding)
```

We used this process for examining other covariates of interest that might logically confound the relationship between our outcomes and main exposure. 

Finally, we were also interested in examining whether or not a focus on healthcare policy affected spending per claim. We had originally added the IV to our regression before realizing that it's significance warranted further research. Using simple linear regression we found that our Instrumental Variable (Medicaid Expansion status in 2017) did not have a significant impact on inpatient or total spending. However, it was significant in our final analyses when looking at hospital complications. 
```{r}
mod_spending <- lm(total_spend ~ hc_policy_focused_state, data = master)
summary(mod_spending)
mod_ip_spending <- lm(ip_spend ~ hc_policy_focused_state, data = master)
summary(mod_ip_spending)
...
The analyses to follow dig into the relationship between two outcome measures (Postoperative Complications and Responsiveness of Hospital Staff) and Costs. For each outcome measure, we do regressions, estimate confidence intervals, segment by covariates, and visualize.
