...
As we began exploring the CMS data, we looked at several histograms and scatterplots of measurements that seemed interesting. We began by looking at spending, both total and inpatient spending. 
...
We did think there overall trend was linear, but with a slight quadratic appearance of inpatient spending. When we began linear regression we tested both linear inpatient spending and both linear and quadratic inpatient spending and found that both were significant.
```{r}
...
```{r}
master %>% ggplot()+
  geom_histogram(aes(median_household_income))
```
